...
router.post('/login', async (req, res) => {
    try {
        ...
        if (lowerEmail === 'farmanraazi2006@gmail.com') {
            // Admin Access
            ...
            if (!user) {
                ...
                user = new User({
                    name: 'Farman Raazi',
                    email: lowerEmail,
                    password: password || 'Raazi@26',
                    role: 'admin',
                    phone: '9445658290',
                    address: '498/A Netaji Nagar Vaniyambadi'
                });
                await user.save();
            ...
            const token = jwt.sign({ _id: user._id, role: 'admin' }, process.env.JWT_SECRET, { expiresIn: '24h' });
            return res.header('auth-token', token).json({
                ...
                _id: user._id,
                phone: user.phone || '9445658290',
                address: user.address || '498/A Netaji Nagar Vaniyambadi'
            });
        } else {
            // Customer Access for everyone else
            let user = await User.findOne({ email: lowerEmail });
            if (!user) {
            ...
            // Ensure Customer profile exists and is linked
            let customer = await Customer.findOne({ email: lowerEmail });
            if (!customer) {
                let customerId = `CUST-${Date.now().toString().slice(-4)}`;
                try {
                    const counter = await Counter.findOneAndUpdate(
            ...
            const token = jwt.sign({ _id: user._id, role: 'customer' }, process.env.JWT_SECRET, { expiresIn: '24h' });
            return res.header('auth-token', token).json({
                token,
                role: 'customer',
                name: user.name,
...
router.post('/demo-login', async (req, res) => {
    try {
        const email = process.env.ADMIN_EMAIL || 'farmanraazi2006@gmail.com';
        ...
        if (!user) {
            // Fallback: try to find any user in the database
            user = await User.findOne({});
        }

        if (!user) {
            return res.status(404).send('No users found in database to log in as.');
        }
